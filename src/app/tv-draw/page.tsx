...
export default function TVDrawPage() {
  ...
  // Listen for draw-specific socket events
  useEffect(() => {
    const socket = getSocket();

    socket.on('drawPhase', (data: { timer: number }) => {
    ...
    socket.on('mmLiquidationBonus', (data: { nickname: string; amount: number }) => {
      ...
      setTimeout(() => setMmLiquidationEvent(''), 3000);
    });
  ...
  // Reset MM earnings on new round
  useEffect(() => {
  ...
  // Auto-create draw room — wait for socket connection to avoid race condition
  ...
  if (!gameState) {
    return (
      <div className="h-screen bg-background flex items-center justify-center">
        <div className="text-accent-purple text-4xl font-display animate-pulse">Загрузка...</div>
      </div>
    );
  }
    ...
    return (
      <div className="h-screen bg-background text-white flex flex-col">
        <header className="text-center py-8">
          ...
          <p className="text-accent-purple text-xl mt-2 inline-flex items-center gap-2" style={{ textShadow: '0 0 20px rgba(179,136,255,0.4)' }}>
            DRAW MODE
          </p>
        </header>
        ...
        <div className="flex-1 flex items-center justify-center gap-16 px-8">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-5xl font-display font-mono font-bold text-accent-purple tracking-widest">{roomCode}</p>
              ...
              <ul className="space-y-2">
                {playerNames.map((name) => (
                  <li key={name} className="text-xl text-white flex items-center gap-2">
                    <span className="text-accent-purple">●</span> {name}
                  </li>
                ))}
              </ul>
            )}
        ...
        <footer className="text-center py-6">
          {playerNames.length >= 2 ? (
            <button
              onClick={startGame}
              className="bg-accent-purple text-white font-display font-bold text-2xl px-12 py-4 rounded-xl hover:bg-accent-purple/90 transition-all hover:scale-105 active:scale-95 glow-purple animate-glow-pulse"
            >
              СТАРТ
            </button>
          ) : (
            <p className="text-text-muted text-xl">Минимум 2 игрока</p>
          )}
        </footer>
      </div>
    );
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-border">
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2 bg-surface-light px-4 py-2 rounded-lg">
              ...
              <span className="text-accent-purple text-3xl font-mono font-black" style={{ textShadow: '0 0 20px rgba(179,136,255,0.4)' }}>
                {drawTimer}с
              </span>
            </div>
            <span className="text-text-secondary text-lg">
              MM: <span className="text-accent-purple font-bold">{mmNickname}</span> рисует...
            </span>
          </div>
        </div>
        ...
        <div className="flex-1 flex">
          <div className="flex-1 p-4 flex items-center justify-center">
            <div className="text-center">
              ...
              <svg width="80" height="80" viewBox="0 0 24 24" fill="none" className="mx-auto mb-6 animate-bounce">
                <path d="M17 3a2.828 2.828 0 1 1 4 4L7.5 20.5 2 22l1.5-5.5L17 3z" stroke="#B388FF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
              <p className="text-4xl font-display font-bold text-accent-purple mb-2" style={{ textShadow: '0 0 30px rgba(179,136,255,0.3)' }}>
                Маркет-мейкер рисует график...
              </p>
              <p className="text-text-secondary text-xl">
                {mmNickname} создаёт следующий раунд
              </p>
            </div>
          </div>
  ...
  if (phase === 'countdown') {
    return (
      <div className="h-screen bg-background flex flex-col items-center justify-center">
        ...
        <p className="text-accent-purple text-2xl mb-4">{ticker}</p>
        <div className="text-[200px] font-display font-black text-accent-purple animate-countdown leading-none" style={{ textShadow: '0 0 80px rgba(179,136,255,0.5)' }}>
          {countdown}
        </div>
      </div>
    );
  ...
  if (phase === 'trading') {
    return (
      <div className="h-screen bg-background flex flex-col text-white">
        ...
        {/* MM liquidation bonus alert */}
        {mmLiquidationEvent && (
          <div className="absolute top-16 left-1/2 -translate-x-1/2 bg-accent-purple text-white font-bold text-xl px-6 py-2 rounded-xl z-50 animate-alert">
            {mmLiquidationEvent}
          </div>
        )}
        ...
        <div className="flex items-center justify-between px-6 py-3 border-b border-border">
          ...
          <div className="flex items-center gap-4">
            {/* MM earnings tracker */}
            <div className="flex items-center gap-2 bg-accent-purple/10 border border-accent-purple/30 px-3 py-1.5 rounded-lg">
              <span className="text-accent-purple text-sm font-bold">MM</span>
              <span className="text-accent-purple font-mono font-bold text-lg">+${mmEarnings.toFixed(0)}</span>
            </div>
              ...
              {stats.map((s) => (
                  <tr key={s.nickname} className={`border-t border-border ${s.rank === 1 ? 'text-accent-gold text-xl' : 'text-lg'}`}>
                    ...
                    <td className="py-4 font-bold">
                      {s.nickname}
                      {s.role === 'market_maker' && (
                        <span className="text-accent-purple text-sm ml-2 px-1.5 py-0.5 bg-accent-purple/20 rounded">MM</span>
                      )}
                    </td>
