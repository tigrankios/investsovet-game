...
function PlayContent() {
  ...
  const {
    ...
    joinRoom, openPosition, closePosition, usePlayerSkill, mmPush, spinSlots, spinWheel, openLootbox, playLoto, voteNextRound,
  } = useGame();
    ...
    return (
      <div className="min-h-screen bg-background text-white flex flex-col">
        ...
        {!isBlind && candles.length > 0 && (
          <div className="mx-4 mt-2 h-32 rounded-xl overflow-hidden">
...
function MiniChart({ candles, positions = [] }: { candles: Candle[]; positions?: import('@/lib/types').LeaderboardEntry[] }) {
  ...
  return (
    <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-full">
      <rect x={0} y={0} width={width} height={height} fill="#0B0E17" rx={6} />
      {visible.map((c, i) => {
